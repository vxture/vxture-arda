/**
 * GET /api/v1/quality-rules - list quality rule definitions (tenant data,
 * force-filtered by the key's workspace).
...
import { listQualityRules } from "../../../lib/reads/quality";
...
export async function GET(req: NextRequest) {
  ...
  const result = await listQualityRules(auth.workspaceId, {
    limit: page.limit,
    cursorId: page.cursorId,
    datasetId: parsed.value.datasetId,
    enabled: parsed.value.enabled === undefined ? undefined : parsed.value.enabled === "true",
  });
