...
// ALL platform-integration config for at-a-glance inspection (mirrors karda's
// lib/status.ts - same C1/C2/C3 shape, since arda runs the same App
...
export function buildStatus(env: Env, now: string): IntegrationStatus {
  ...
  return {
    identity: {
      productCode: serviceIdentity({ service: "arda-app", product: "arda" }).service,
      gitSha: serviceIdentity({ service: "arda-app", product: "arda" }).gitSha,
      appEnv: env.NEXT_PUBLIC_APP_ENV ?? env.NODE_ENV ?? "unknown",
      time: now,
    },
    c1: {
      enabled: Boolean(env.OIDC_ISSUER && env.OIDC_CLIENT_SECRET),
