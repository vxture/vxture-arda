...
> 上游：[`data-100`](arda-data-100-architecture.md)（总体架构）；相关横切 `data-110`（隔离）/ `data-120`（索引）/ [`data-140`](arda-data-140-audit.md)（审计与幂等）；对应板块 [`biz-220`](arda-biz-220-integration.md)（集成）/ [`biz-230`](arda-biz-230-governance.md)（治理）/ [`biz-250`](arda-biz-250-admin.md)（管理）；安全规范 [`30-security.md`](../20-specs/30-security.md)
...
连接串与凭据**不是** schema 字段，而是 `.env` 运行时值。SoT（`prisma/schema.prisma`）里的 `datasource db { provider = "postgresql" }` 不含内联连接串；连接信息由环境变量注入（见 [`data-100`](arda-data-100-architecture.md) §2 与 [`30-security.md`](../20-specs/30-security.md) "Secret Management"）。
...
约束（来自 `CLAUDE.md` 仓库卫生 + `.env.example` 头注 + [`30-security.md`](../20-specs/30-security.md)）：
...
5. secrets 键增删同步 `.env.example`、[`30-security.md`](../20-specs/30-security.md) "Secret Management" 与本文 §3.4；真值绝不入 Git。
