...
> 状态：权威设计 · 已落地（只读面 v1；写操作与身份透传见 §6 路线）
...
- **scope 是 fail-closed 白名单**：词表在 `lib/api/scopes.ts`（`catalog:read` / `quality:read` / `lineage:read`）。存量空 `scopes` 的 key 只能继续用 DataService 网关，不能进 `/api/v1`。
...
后续（第二步/第三步，顺序既定）：写端点（`Idempotency-Key` 头 + `AuditLog.idempotencyKey` 落地）、写 scope 词表、token exchange 身份透传（RFC 8693，替代/并行 ApiKey）、`.well-known/vxture-tools` 工具清单填充（面向任务的粗粒度工具，非 CRUD 镜像）。
