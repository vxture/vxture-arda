...
> 上游：本系列 `biz-100`/`biz-2xx`/`biz-4xx`；现状证据 `arda-functional-domains-and-entitlement.md` §0、[`data-300`](arda-data-300-migration.md)
...
### 阶段 2 · 共性断链（一处补、多功能受益）
- **审计写入**统一封装：✅ 已收口（2026-07-17，每个 `"use server"` 变更动作均落 `AuditLog`）：`Q-BL3`/`Sec-BL3`/`Lc-BL4`/`S-BL4`/`M-BL4`/`L-BL4`/`MD-BL6`。`Svc-BL5` 仅剩服务变更（`DataService` 创建/编辑）子项，卡在该功能本身还没建 UI。
- **调度**（scheduling）：质量跑批 `Q-BL1`、接入同步 `I-BL2`、血缘采集 `L-BL2`——刻意推迟到有真实需求再做，不提前建。
...
- 平台侧依赖（权益端点/角色词表/指令通道）见 [`20-vxture-platform-integration-requirements.md`](../70-workplan/20-vxture-platform-integration-requirements.md)（`plat` 维度）。
