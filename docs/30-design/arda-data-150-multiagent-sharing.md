...
```
              ...
              approved standards / admin-division codes / currency codes
                 workspaceId = NULL (显式轴: NULL=平台全局, 只读, 单一权威)
                                    |
             read overlay:  workspaceId = self OR workspaceId IS NULL
             +----------------------+----------------------+
             |                                             |
   +-------------------------------+       +-------------------------------+
...
| 层 | 归属 | 落点 | 可见性 | 谁可写 |
|---|---|---|---|---|
| **平台层** | arda 运营策展的全局参考数据 | arda（`scope=platform`，显式轴 `workspaceId=NULL`） | 全平台**只读**共享（叠加进每个 workspace 读） | 仅 ops/平台角色 |
...
|     -> 平台层:  scope=platform, workspaceId=NULL, 仅 ops 可写
|
...
平台层是本轮唯一在隔离硬轴之外「向上叠加」的机制，需谨慎设计。它由四件事构成：`AssetScope` 枚举、平台全局行（`workspaceId IS NULL`）、读叠加、ops 写角色门与升格流。

### 4.1 AssetScope 枚举与平台全局行
...
`scope=platform` 的行用**显式轴** `workspaceId = NULL`（NULL=平台全局）落库。关键点：`workspaceId` 是普通索引列、**非外键**（见 [`data-110`](arda-data-110-isolation.md) §1），因此平台全局行不需要先存在一条 `WorkspaceRef` 行 - 它就是「不属于任何 workspace」的显式表达（`scope=platform` 与 `workspaceId IS NULL` 由 CHECK 约束保持一致）。这也遵守「workspace_id 平台签发」铁律：产品不再自造哨兵值（data_platform_100 §2.3.2）。schema 顶部 `AssetScope` 的注释即锁定此设计：`platform` = arda-ops-curated 全局参考、对**所有** workspace 只读，`workspace` = 租户自有、隔离在本 workspace 内并只在其 agent 之间共享。

### 4.2 读叠加：workspaceId = self OR workspaceId IS NULL

租户读取带 `scope` 的资产时，把过滤从「等于自己」放宽为「自己**或**平台全局行」：
...
这样每个 workspace 都能读到全平台统一的那份平台层参考，而写路径仍严格 `workspaceId = self`（租户无法写平台全局行）。**注意适用范围**：读叠加只对承载 `scope` 的表启用（本轮为 `GlossaryTerm`、`Standard`、`Dataset`，见 §7）；其余业务表不引入平台全局行、不放宽读过滤，隔离语义完全不变。
...
真实 workspaceId          (仅 ops 可执行)           workspaceId=NULL
```
...
- **D4**：平台层用显式轴 `workspaceId=NULL`（NULL=平台全局，普通列非 FK，无需 WorkspaceRef 行；`scope=platform` 与之由 CHECK 一致），租户读叠加 `workspaceId = self OR workspaceId IS NULL`，写 `scope=platform` 仅 ops/平台角色。
