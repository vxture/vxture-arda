...
>
> **[2026-07-13 取代：能力/配额分权（owner 裁定，松耦合定稿）]** 本文中「features / capability_keys 由平台按档配置并经 C2 `capabilities` 下发」的所有表述**作废**：**功能语义归产品**（哪档解锁什么功能 = 产品仓库内的能力矩阵，平台不配置、不感知功能键），**配额归平台**（上限数字与消耗池 = 销售策略，workspace 级、跨产品，§11.5 瀑布扣减等机制原样保留）。相应地：§11.2 `Product.capability_keys` 与 `Plan.components[].features` 从平台模型移除；§11.3 路 A 的 `features = union(...)` 作废（平台只做 tier 就高合并与上限就高合并）；§11.7 响应体的 `capabilities.features` 移除、上限型数字改独立 `limits` 块。权威契约形状见 arda 仓 [`ent-120`](20-design/arda-ent-120-consumption-contract.md) v2 与回函 06；quota_pools/consume/invalidate 不受影响。
>
...
§3.5（实时拉取+缓存+失效）、§1（SoR 边界）**保留不变**；§3.4 的分工~~（features 键产品定义、档位映射平台下发）~~已于 2026-07-13 取代为「能力全归产品、配额全归平台」（见文首取代注）。
...
- EntitlementGate 按 `status` 二元放行（`active` 放行，`expired`/`none` 一律拒绝）；~~§3.4「无 free 特例，按 features/quota 渲染放行」~~ **已推翻（2026-07-03，决策 A）**，见 `ADR-entitlement-and-workspace.md` §3.4 修正注。
