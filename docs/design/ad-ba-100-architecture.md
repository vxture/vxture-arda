# arda 业务能力总体架构（ad-ba-100-architecture）

> 状态：第 1 层 · 总体架构（待评审）
> 范围：arda 数据域的**业务能力全景、板块划分、价值链与跨切面**；不下钻到板块内部（见 `ba-210..250`）
...
**只做数据域**：kb、业务智能体、RAG/向量、LLM、交付物生成属其他产品（见 `ba-000` §0）。arda 提供数据，不做智能体。
...
> 补齐维度的实现成熟度不一：血缘 / 安全 / 元数据已有实体或屏幕（见 `ba-210`/`ba-230`）；**生命周期（留存 / 销毁）、存储、主数据目前薄或空**（`arda.quota.history_retention_days`/`storage_bytes` 已有键，wipe 见 ADR §5.1 未实现；主数据见下）——落地见 `ba-300`。
...
- **主数据归 `governance`，不归 `assets`**（理由见 `ba-230` §1）：资产域=有哪些数据；治理域=哪份权威 / 可信。
...
> 采纳影响：`ba-210`（资产域=结果）纳入各能力的**只读结果面**；`ba-230`（治理域=过程+目标）保留**执行/定义面**；并新增**能力专题（纵向）**作为二级导航。板块重排见 `ba-110` §5 与 `ba-300`。
...
| 板块 | 编号 | 一句话范围 |
|---|---|---|
| 数据资产 | [`ba-210`](ad-ba-210-assets.md) | 编目、元数据、标签/术语、资产详情 |
| 数据集成 | [`ba-220`](ad-ba-220-integration.md) | 数据源登记（v1）、管道/调度（future） |
| 数据治理 | [`ba-230`](ad-ba-230-governance.md) | 标准、质量、血缘、安全策略、分级 |
| 数据服务 | [`ba-240`](ad-ba-240-services.md) | 服务/API 目录、发布、对外共享 |
| 管理 | [`ba-250`](ad-ba-250-admin.md) | API Key、审计日志 |

落地路径（现状→目标）见第 3 层 [`ba-300`](ad-ba-300-implementation.md)。
