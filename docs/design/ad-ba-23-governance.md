...
> **参考与主数据**（`ba-10` §1）：参考数据（代码集/数据元）已由 `Standard` 覆盖；**主数据（MDM）**是治理层的补齐维度——轻量落地=把主数据 `Dataset` 标为权威金记录 + steward + 质量规则 + 主数据服务；重型 MDM 引擎（匹配/合并/survivorship）深度待定（见 §7 待办与 `ba-10` §1）。
