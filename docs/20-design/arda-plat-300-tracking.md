...
| 11 | 信封 v2 切换 + `PLATFORM_API_URL` 别名切换 + `AUTH_INTERNAL_TOKEN` 轮换 | ~~等平台通知~~ **平台已一步切完（`arda_305_reply-05` cutover 函，2026-07-14）**：别名 `http://100.100.197.42:8080`、v2 已生效（v1 不再下发）、scope+claim 已退役（reseed 生效，**arda 请求退役 scope 会 invalid_scope 拒登**） | **执行中**：代码侧 scope 修正 + EnvGuard 移除（本 PR）；host 侧两栈 `PLATFORM_API_URL` 切别名 + 验证清单（arda_305 §4）；token 轮换值待 owner 带外转运（不阻塞，现值有效） |
