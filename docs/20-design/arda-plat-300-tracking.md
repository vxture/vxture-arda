...
**【2026-07-13 收口：议题整体撤回】**回函 05 被回函 06 撤回——owner 裁定权益契约松耦合改形（能力归产品矩阵 / 配额归平台 / 商业决策 UI 归 console 深链），`null` 态 CTA 分岔由 console 渲染解决，**三个方案均不再需要**；`arda:subscription` scope 退役**解锁、无前置**，并进一步提案 `arda` claim 整体退役。详见 `docs/70-reply/arda-plat-250-loose-coupling-reply-2607132030.md` + [`ent-120`](arda-ent-120-consumption-contract.md) v2。
...
| 10 | 升级 `@vxture/shared@^1.4.0`（六值 status，arda_303 §7.1） | **[owner 操作] 本地 `NODE_AUTH_TOKEN` 已失效（npm 401）**，需换发含 `read:packages` 的 PAT | 阻塞中：caret 范围已兼容 1.4.x，token 恢复后 `npm update @vxture/shared` + 重跑验证即可；`hasProductAccess` 已前向就绪（`overdue` 放行，1.3.1 下惰性）|
| 11 | 信封 v2 切换 + `PLATFORM_API_URL` 别名切换 + `AUTH_INTERNAL_TOKEN` 轮换 | 平台另函通知（arda_303 §5/§6#2-4，三项并窗） | 等平台通知，arda 无需预动作（消费端已 v1/v2 双容）|
| 12 | **EnvGuard 退役/中性化**（claim 退役的连带项，arda_303 §7 未列、arda 自查发现） | 与 #11 的 claim 退役同窗 | 待实施：`arda` claim 退役后 `EnvGuard` 唯一数据源（`ardaClaim.state`）消失；beta 已内部化、用户路径无 state 路由需求，同窗移除 EnvGuard 并把 `subscriptionFromClaim` 收敛为纯 env mock |
...
| 2026-07-13 | **权益契约松耦合定稿（owner 裁定）+ 回函 06 签发**：C2 信封 v2（删 `capabilities`、上限挪 `limits`、加时间戳），能力归产品能力矩阵、配额仍全归平台；契约演进边界规则（决策位禁入信封）；回函 05（had_trial）撤回、`arda:subscription` scope 退役解锁、`arda` claim 整体退役提案；附平台服务边界建议（commerce 拆出 auth-bff + 中性网关别名）。arda 侧文档已同步修订（ADR §3.4 / ADR-11 / ent-100/110/120 / biz-260 / feature-keys / decisions.md） |
