...
> 上游：`entitlement.md`（现状实现全文）、`ADR-entitlement-and-workspace.md` §3.1-3.2（仅作 arda 需理解的契约形状）；代码真源 `portals/app/app/entitlement/*.ts(x)`、`portals/app/app/auth/lib/claims.ts`
...
> **`docs/20-design/entitlement.md` 的历史漂移已修正**（2026-07-03）：该文件此前仍描述旧的 4 档 `free(0)/pro(1)/team(2)/enterprise(3)`（无 `starter`/`business`），与代码现状（5 档）不一致；已同步更新为本文件描述的 5 档模型。后续仍建议以本文件为唯一权威，`entitlement.md` 仅作实现细节补充（见 [`ent-300`](arda-ent-300-migration.md) §2）。
...
**代码真源**：`portals/app/app/auth/lib/claims.ts` 的 `toArdaClaim()`。这是 `entitlement.md` 完全没有记录、但已经写进代码的关键事实——arda 同时兼容平台**当前真实格式**与**未来目标格式**：
...
| 维度 | 现状（代码 + `entitlement.md`） | 目标（`ADR-entitlement-and-workspace.md` §3.1-3.2，仅取契约形状） |
|---|---|---|
