...
> 一句话定位：**workspace 是硬隔离边界（不变），agent 是软治理轴（新增）；arda 只做元数据中介与网关，内容字节从不在 arda 静置，跨 workspace 永不流动。**
...
- **workspace = 硬隔离边界**：一切业务行带 `workspaceId`，服务端 `where: { workspaceId }` 收口。跨 workspace 的数据可见性为零。
...
```
   ...
   ///  = workspace 硬墙，数据永不跨越 (无 share-grant 原语)。
```
...
1. **跨 workspace 永不流动**：本设计**没有 share-grant 原语**（有意被设计掉，这是最大简化）。共享面止于 workspace 内的多个 agent；不存在「把 A workspace 的 Dataset 授权给 B workspace」的机制。任何跨 workspace 的数据需求都不通过 arda 的共享层满足。
...
| 轴 | 字段 | 性质 | 作用 |
|---|---|---|---|
| **硬隔离轴（主轴，不变）** | `workspaceId` | 强制行级过滤，普通列非 FK | 唯一的隔离边界；一切安全性兜底于此 |
...
- **D1**：workspace = 硬隔离边界，`workspaceId` 行级强制过滤不变；一切安全性兜底于此。
...
- **D8**：跨 workspace 永不流动 - 无 share-grant 原语（有意被设计掉，最大简化）；共享面止于 workspace 内多 agent。
...
5. 决策增删改（§9 的 D1..Dn）须保留编号连续与一句话粒度，重大反转应在 [`decisions.md`](decisions.md) 留痕并回链本文件。
