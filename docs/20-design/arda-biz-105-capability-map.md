...
> [`domain-entities-and-feature-keys.md`](domain-entities-and-feature-keys.md) (能力键目录)
...
先在 `domain-entities-and-feature-keys.md` SS3.1 登记键, 再落到
`capability.ts`)。

> 待办: 上述 4 个新 future 键与 `arda.governance.master_data` 的屏幕映射变更,
> 应在下次修订 `domain-entities-and-feature-keys.md` SS3.1 时补录, 保持"键目录
...
导航; `admin` 域的平台深链接占位; `domain-entities-and-feature-keys.md` 的键目录
补录。
