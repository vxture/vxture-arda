...
> 上游：`vxture-platform-integration-requirements.md` §0-1；`ADR-entitlement-and-workspace.md` §1.7（SoR 分工）
...
> **通道 B 的一个已知问题**：`vxture-platform-integration-requirements.md` §3.1 描述的响应契约是 `ADR-11`（多 Plan/Product 合并模型）定稿**之前**的旧扁平格式，与 [`ent-120`](arda-ent-120-consumption-contract.md) 已经写定的权威契约（`capabilities`/`quota_pools` 分层结构）不一致。以 `ent-120` 为准；旧格式的处理见 [`plat-300`](arda-plat-300-tracking.md) §2。
